...
// Sender can only be called from a QAudioProbe that was directly constructed.
func (this *QAudioProbe) Sender() *qt.QObject {

	var _dynamic_cast_ok C.bool = false
	_method_ret := qt.UnsafeNewQObject(unsafe.Pointer(C.QAudioProbe_protectedbase_sender(&_dynamic_cast_ok, unsafe.Pointer(this.h))))
...
// SenderSignalIndex can only be called from a QAudioProbe that was directly constructed.
func (this *QAudioProbe) SenderSignalIndex() int {

	var _dynamic_cast_ok C.bool = false
	_method_ret := (int)(C.QAudioProbe_protectedbase_senderSignalIndex(&_dynamic_cast_ok, unsafe.Pointer(this.h)))
...
// Receivers can only be called from a QAudioProbe that was directly constructed.
func (this *QAudioProbe) Receivers(signal string) int {
	signal_Cstring := C.CString(signal)
	defer C.free(unsafe.Pointer(signal_Cstring))

	var _dynamic_cast_ok C.bool = false
	_method_ret := (int)(C.QAudioProbe_protectedbase_receivers(&_dynamic_cast_ok, unsafe.Pointer(this.h), signal_Cstring))
...
// IsSignalConnected can only be called from a QAudioProbe that was directly constructed.
func (this *QAudioProbe) IsSignalConnected(signal *qt.QMetaMethod) bool {

	var _dynamic_cast_ok C.bool = false
	_method_ret := (bool)(C.QAudioProbe_protectedbase_isSignalConnected(&_dynamic_cast_ok, unsafe.Pointer(this.h), (*C.QMetaMethod)(signal.UnsafePointer())))
